...
// Function to update countdown to 2:40
function updateCountdown() {
  const now = new Date();
  const targetTime = new Date(now);
  targetTime.setHours(14, 39, 57, 0); // Set the target time to 14:39:57
  
  let timeDifference = targetTime.getTime() - now.getTime();

  if (timeDifference < 0) {
      // If the target time has passed, set the countdown text to "00:00:00.000"
      document.getElementById('countdown').textContent = 'Enjoy Febuary Break!!!';
  } else {
      ...
      // Format the countdown text with leading zeros
      hours = hours.toString().padStart(2, '0');
      minutes = minutes.toString().padStart(2, '0');
      seconds = seconds.toString().padStart(2, '0');
      milliseconds = milliseconds.toString().padStart(3, '0');

      // Update the countdown div with the formatted time
      document.getElementById('countdown').textContent = `Countdown to Break: ${hours}:${minutes}:${seconds}.${milliseconds}`;
  }
}
...
// Initial call to display the time and countdown immediately
updateTime();
updateCountdown();
